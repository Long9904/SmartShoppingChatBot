...
public sealed class GetConversationProductComparisonsQueryHandler(
    ICurrentUserService currentUserService,
    ICustomerRepository customerRepository,
    IConversationRepository conversationRepository,
    IProductComparationRepository productComparationRepository)
    : IRequestHandler<GetConversationProductComparisonsQuery, Result<CursorPage<ProductComparisonResponse>>>
{
    public async Task<Result<CursorPage<ProductComparisonResponse>>> Handle(
        GetConversationProductComparisonsQuery request,
        CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.Filter.LastCursor))
        {
            ...
            {
                return Failure(400, "Invalid product comparison cursor.", MessageCodeForMessage.InvalidCursor);
            }
        ...
        {
            return Result<CursorPage<ProductComparisonResponse>>.Failure(
        ...
        var page = await productComparationRepository.CursorPagingAsync(
            businessId,
            conversationId,
        ...
        return Result<CursorPage<ProductComparisonResponse>>.Success(
            new CursorPage<ProductComparisonResponse>
            {
                Items = page.Items.Select(ProductComparisonResponse.FromEntity).ToList(),
                HasMore = page.HasMore,
                NextCursor = page.NextCursor
            },
            200,
            "Get conversation product comparisons successfully.",
            ConversationMessageCode.Success);
    }

    private static Result<CursorPage<ProductComparisonResponse>> Failure(
        int statusCode,
        string message,
        string messageCode) => Result<CursorPage<ProductComparisonResponse>>.Failure(
            statusCode,
            message,
